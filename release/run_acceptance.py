...
SAFE_ENV_NAMES = frozenset({"PATH", "HOME", "USER", "LOGNAME", "SHELL", "LANG", "LC_ALL", "CI", "TERM", "TMPDIR", "TMP", "TEMP", "CHROME_BIN", "PYTHON", "NODE", "COREPACK_ENABLE_PROJECT_SPEC", "NPM_CONFIG_STORE_DIR", "GITHUB_ACTIONS", "GITHUB_WORKFLOW"})
...
def acceptance_exit_code(command_results: dict[str, dict], browser_manifest_blocks: list[str]) -> int:
    expected = {command.name for command in RUNNER_COMMANDS}
    if set(command_results) != expected or browser_manifest_blocks:
        return 1
    return 0 if all(
        record.get("result") == "pass" and record.get("exit_code") == 0
        for record in command_results.values()
    ) else 1
...
def substitute(argv: tuple[str, ...], output: Path, wheel: Path | None = None) -> tuple[str, ...]:
    ...
    resolved = tuple(item.replace("{OUTPUT}", values["{OUTPUT}"]).replace("{WHEEL}", values["{WHEEL}"]) for item in argv)
    return (sys.executable, *resolved[1:]) if resolved and resolved[0] == "python" else resolved
...
def main() -> int:
    ...
    return acceptance_exit_code(command_results, browser_manifest_blocks)
