...
Quillframe 当前的 OpenAI Chat 生产调用全部要求结构化 JSON，因此请求体显式发送 `response_format: {"type":"json_object"}`，并使用标准 SSE 流式响应，直到收到完成原因或 `[DONE]`。Core 只组装 `delta.content`，不会把 `reasoning_content` 纳入结果。根据 Z.AI 当前公开的 Chat Completion 契约，精确模型 ID `glm-5.3` 与 `glm-5.3-flash` 只能开启 thinking，且支持 `low | high | max`；对应请求显式发送 `thinking.enabled + reasoning_effort=low`，避免默认 `max` 挤占类型化输出。这个映射只属于已核实的 wire compatibility，不推断小说能力。系统仍验证返回的精确 Rust 类型；JSON、流式或 reasoning 参数不会授予语义正确性或权威。
