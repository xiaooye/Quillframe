...
`quality_evolution.py` 为基准稿和挑战稿保存精确内容指纹；语义上的胜者由已登记的 `quality.compare` 判断。确定性账本只负责验证比较双方、目标约束集和一次性消费；只有语义结果选择挑战稿时才更新基准稿。它不会根据机械的连续记录自动停机，结束必须显式发生。
...
管理器自审不等于独立评审；机械元数据也不等于语义判断。有效的语义拒绝是正式判断，不是基础设施故障；系统必须回到真正负责问题的修订机制，不能靠不停更换评审者来规避结果。
