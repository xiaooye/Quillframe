...
All current Quillframe OpenAI Chat production calls require structured JSON, so the request body explicitly sends `response_format: {"type":"json_object"}` and uses standard SSE streaming until a finish reason or `[DONE]`. Core assembles only `delta.content`; `reasoning_content` never enters the result. Under Z.AI's current published Chat Completion contract, the exact model IDs `glm-5.3` and `glm-5.3-flash` require thinking and support `low | high | max`; those requests explicitly send enabled thinking with `reasoning_effort=low` so the default `max` does not consume the typed-output budget. This is a verified wire-compatibility mapping, not an inference of fiction capability. Core still validates the exact Rust result type, and JSON, streaming, or reasoning controls grant neither semantic correctness nor authority.
