...
- Surface 单场景请求不再重复携带整章作者指令、全章 `must_happen` 或非末场的章尾债务，避免模型在每个场景重演整章；末场仍精确接收停笔点与章尾债务，全程仍继承禁写项、时间锚、项目指导与当前场景合同。Surface 对象若只有 JSON 字符串内部的换行／回车／制表符未转义，Core 可在确认整个响应是单一精确对象后仅转义这些控制符再执行原类型校验；任何尾随 `User`／`Assistant` 内容、第二 JSON 值、残缺对象或其他控制字符继续拒绝。
