...
- 模型端点现在保留显式 `v1`／`v4`／`v4.1` API 基址，只在没有版本末段时补入默认 `v1`，避免发现模型或推理时静默改写 Provider 路径。生产模型调用统一使用文档声明的 180 秒截止时间；人物模拟与场景解析的结构化输出上限收紧到 3000 tokens，避免小型因果制品因过宽生成窗口反复触发 transport timeout。
