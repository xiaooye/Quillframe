...
- Surface 单场景请求不再重复携带整章作者指令、全章 `must_happen`、请求级规则材料或非末场的章尾债务，避免模型在每个场景重演整章；末场仍精确接收停笔点与章尾债务，全程仍继承禁写项、时间锚、项目指导与当前场景合同。Surface 对象若只有 JSON 字符串内部的换行／回车／制表符未转义，Core 可在确认整个响应是单一精确对象后仅转义这些控制符再执行原类型校验；Provider 若先输出原始正文、再输出正文逐字相同的完整 `{manuscript}` 对象，Core 会隔离重复前缀并只保留一个类型值；前后正文只要不同仍拒绝。Repair Editor 两种已观察到的固定键引号损坏仅在整个响应仍是单一精确对象且修复后的 typed repair 合同通过验证时规范化。有界 replacement 数组只有在完整覆盖一组唯一逐字 source identity 时才可机械重排；明确写为“无需修改”的 no-op target 转为保护边界，真实窗口缺失或重复仍拒绝。非失败 Surface assessment 上无意义的 repair scope 只会被归一为空，不改变其判断。任何尾随 `User`／`Assistant` 内容、第二 JSON 值、残缺对象或其他控制字符继续拒绝。
