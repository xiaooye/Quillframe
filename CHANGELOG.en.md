...
- Model endpoints now preserve explicit `v1`/`v4`/`v4.1` API bases and add the default `v1` only when no version segment is present, preventing model discovery or inference from silently rewriting provider paths. Production calls now use the documented 180-second deadline, while character-simulation and scene-resolution structured outputs are bounded to 3,000 tokens so small causal artifacts do not inherit an unnecessarily broad generation window.
