...
当前 Rust-native 生产会把本文件全文、规则 ID 集、登记语义 rubric 与项目指导冻结成生产指导快照。Writer 只读取正向实现指导，不读取这份负面检查表；候选冻结后，专用 Surface Auditor 必须按 `HF-01..HF-30` 顺序各返回一次绑定正文证据的 assessment。确定性 Core 只核验规则覆盖、指纹、证据引用与结论自洽，不按词表或句长统计决定文学失败。缺项、重复、未知规则、证据不足或确认失败都会阻断用户可见 release。
