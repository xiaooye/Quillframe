...
Surface Fundamentals / 表层基本规则是 NovelForge 默认的正文实现安全层。它存在，是因为语言模型会跨项目反复出现一批稳定失败机制：用碎句假装节奏、把连续动作切成镜头、叙述者替场景总结意义、设计语言泄漏、空洞微动作、流程播报、虚假意义感，以及许多“看起来很会写、实际上没有小说功能”的漂亮句子。
