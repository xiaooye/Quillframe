...
`Context Freeze → Story/Canon Preflight → private Character Enactment → Scene Resolution → Scene Realization Contract + model-composed Writer Pack → Reader Pressure → direct Surface Writer → freeze candidate fingerprint → complete Surface Fundamentals semantic audit → Reader Engagement → Continuity → objective-bound self-audit/qualification → owning-layer repair/comparison when needed → independent review → User-visible Gate`

Rejected prose, reviewer analysis, private character deliberation and hidden expected labels stay out of a fresh Writer pack. Every new run freezes the Framework's positive Writer guidance, complete Surface Fundamentals, project guidance materialized natively or supplied explicitly from approved prose sources with matching fingerprints, and the registered semantic-audit contract into one production-guidance snapshot. Writer receives only the positive projection; Surface Auditor must cover `HF-01..HF-30` exactly once. A bounded local repair may receive only its exact fingerprint-bound edit windows, but it cannot replace inherited Framework/project rules and the whole revised candidate is audited again. Deterministic checks verify rule coverage, schema, provenance and boundaries; literary judgment remains with models and the author.
