...
PROJECT_RESOLUTION = ROOT / "project_resolution.py"
PROJECT_LAUNCH = ROOT / "quillframe" / "launch.py"
...
def bootstrap(project_root: Path, task_mode: str) -> dict[str, Any]:
    project_root = project_root.resolve(); code, validation = run_json(PROJECT_RESOLUTION, ["validate", str(project_root)])
    ...
    return {"schema":"quillframe_bootstrap_v1","framework_version":FRAMEWORK_VERSION,"framework_root":str(ROOT),"project_root":str(project_root),"project_id":resolution.get("project_id"),"project_title":resolution.get("project_title"),"language":resolution.get("language"),"chapter_scope":resolution.get("chapter_scope"),"data_root":resolution.get("data_root"),"manifest_fingerprint":resolution.get("manifest_fingerprint"),"task_mode":task_mode,"ready":True,"validation":validation,"project_contract":{"resolver":"project_resolution.py","launch":"quillframe/launch.py","schema":"quillframe_project_v1_0","chapter_scope":"CH001"},"required_framework_reads":["HARNESS_MANIFEST.yaml","SKILL.md","harness/HARNESS_AGENT.md"],"ai_native_policy":"Semantic story/character/reader/revision/research judgment belongs to the model through progressively disclosed model-readable contract packs; deterministic code owns invariants, persistence, routing and transactions.","semantic_entry":"quillframe.py semantic prepare-contract","semantic_catalog":"harness/semantic_workers/model_contract_catalog.json","quality_taxonomy":"quality/taxonomy.json","quality_taxonomy_scan":"quillframe.py quality-taxonomy scan <project-profile-or-regression-files>","repair_policy":"quillframe.py repair-policy evaluate --input <repair-policy-payload>","production_reader_contract":"reader.engagement_audit","production_independent_contract":"quality.production_review","production_independence_evidence":"quillframe_project_peer_validation_receipt_v2 from the Project-owned peer bridge; self-declared session ids do not qualify","task_specific_loading":"Resolve through Project contract + Harness + sparse Context Manifest; never inject the whole project, corpus, or all semantic packs by default.","capability_policy":"Probe/declare host capabilities before routing external/tool work; undeclared capability is unavailable.","authority_policy":"Model outputs, reader diagnostics, memories, branches and semantic results never grant Canon write authority; accepted Canon mutation must pass settlement transaction checks."}

def doctor() -> dict[str, Any]:
    required = [PROJECT_RESOLUTION, PROJECT_LAUNCH, SESSION, MCP, LOCAL_AGENT, PEER_RELAY, REGISTERED_CONTRACT_BINDING, PEER_BRIDGE_RECEIPT, *TOOLS.values(), *CORPUS_TOOLS.values(), SEMANTIC_ROOT / "model_contract_catalog.json", ROOT / "quality" / "taxonomy.json"]
...
def main() -> int:
    ...
    boot=sub.add_parser("bootstrap"); boot.add_argument("--project-root",required=True); boot.add_argument("--task-mode",required=True,choices=TASK_MODES)
    ...
    if args.cmd=="bootstrap": value=bootstrap(Path(args.project_root),args.task_mode); dump(value); return 0 if value["ready"] else 1
    if args.cmd=="project":
        if not args.args: dump({"error":"project subcommand required","examples":["resolve","validate"]}); return 2
        return call(PROJECT_RESOLUTION,args.args)
    if args.cmd=="corpus":
