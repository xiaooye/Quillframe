#!/usr/bin/env python3
"""Synthetic public Actions probe for the exact locked NovelForge release.

Test infrastructure only. This script does not create a real consuming-Project
receipt and does not evaluate novel content. It validates registered-contract,
peer relay, receipt/tamper primitives and the intentional self-host guard using
an exact locked Framework runtime plus this test-only overlay.
"""
...
SYNTHETIC_PROJECT_ID = "PROJECT-PUBLIC-EXACT-TEST"
...
def assert_self_host_guard(*, issue_number: int, framework_commit: str, job: dict) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="novelforge-exact-peer-probe-") as tmp:
        workspace = Path(tmp)
        ...
        (project / "novelforge.toml").write_text(
            "\n".join([
                ...
                'title="Exact release public integration fixture"',
                'language="en"',
                'version="0.0.0-test"',
                'status="active"',
                "[adapter]",
...
def main() -> int:
    ...
    job = make_contract_job(
        ...
        "PUBLIC-EXACT-RELEASE-PROBE",
        {
            "candidate_fingerprint": candidate_fp,
            "candidate_text": "Synthetic exact-release Actions fixture; not novel content.",
            "reader_grip": "very_high",
        },
        source_session_id="SES-PUBLIC-EXACT-MANAGER",
    )
    ...
    result = {
        ...
        "status": "completed",
        "worker": {
            "provider": "other_peer_chat",
            "model_or_reviewer": "synthetic-exact-release-fixture",
        ...
        "judgment": {
            ...
            "result": "pass",
            "codes": [],
            "evidence": ["synthetic exact-release structural fixture"],
    ...
    receipt = build_receipt(
        ...
        runtime_trace={
            ...
            "workflow_name": "NovelForge exact-release public synthetic peer probe",
    ...
    output = {
        "schema": "novelforge_public_exact_release_peer_probe_v1",
        "issue_number": args.issue,
        "framework_commit_under_test": args.framework_commit,
        "test_overlay_only": True,
