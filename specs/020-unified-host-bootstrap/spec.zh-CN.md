...
5. task mode 解析前，Write/Edit/Bash/apply_patch 被拒绝，但极窄的 mode-start command 可执行。
...
- Bash gate 过宽会导致 bootstrap deadlock；pre-mode allowlist 必须只匹配 Quillframe 自己的 deterministic mode/run command。
