...
5. Before task-mode resolution, Write/Edit/Bash/apply_patch are denied but the narrow mode-start command is permitted.
...
- Overly broad Bash denial can deadlock bootstrap; the pre-mode allowlist must match only Quillframe's own deterministic mode/run command.
