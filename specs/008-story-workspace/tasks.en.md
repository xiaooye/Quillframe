...
- [x] T13 · Run deterministic contract / syntax / schema / bundle / Studio typecheck-build / documentation-quality tests and repair feature regressions. Story Workspace reusable CI, semantic contract packs, quality gate, Studio App, generic semantic kernel, bundle and eval stages pass on the integrated branch. Remaining `session-terminate-command` and aggregate Host Bridge dispatch failures reproduce on current `main` and are recorded as upstream blockers rather than 008 regressions.
- [ ] T14 · Obtain independent semantic capability / regression evidence with the blind narrative fixtures; fixture existence and hidden-label isolation alone are not semantic acceptance. Current live gate fails closed before model execution because the repository has no `OPENAI_API_KEY` provider credential.
