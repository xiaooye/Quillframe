...
Ordered scenes retain character action ownership and causal closure. Writer execution may realize one scene or a coherent scene group, but the chapter candidate has one frozen fingerprint. Surface Writer receives one Scene Realization Contract rather than duplicate copies of the full plan lock, chapter plan, and standalone scene brief. The chapter minimum is checked only after assembly and must never be divided into mechanical per-scene minimum quotas.

Every new run freezes a production-guidance snapshot binding the Framework's positive Writer guidance, complete `HF-01..HF-30`, registered semantic-audit rubric, and project prose/voice/style/calibration guidance materialized through native handles from approved Setup sources with matching content fingerprints, or explicitly supplied as the exact approved bytes. Writer receives only the positive projection. A dedicated Surface Auditor returns one evidence-bound assessment for every rule. Missing, duplicate or unknown rules, insufficient evidence, and confirmed failures all block release; deterministic code does not replace semantic judgment with word lists or sentence counts.

Reader Pressure becomes a compact Writer brief and participates in the Writer Pack fingerprint. An empty brief is valid; the system must not force a hook into every chapter. Blind Reader receives only the manuscript and reader-visible positioning, never the brief, plan, rules, expected answers, or treatment identity. Independent review and user-visible output likewise receive no private Writer guidance.

Independent or Surface-audit rejection creates a private repair source for the next REVISE run. A fresh Writer does not receive old reasoning, reviewer chains, or rejected prose. The Editor chooses the owning layer and invalidation boundary. REVISE inherits the source production-guidance snapshot and merges current local requirements by stable rule identity; a narrow repair cannot replace the original Framework, project, or request rules. Every revised whole candidate runs the Surface audit again. REVISE gives Continuity dependency summaries for later accepted chapters; damage produces downstream impact and propagation debt before settlement, never silent Canon edits.
