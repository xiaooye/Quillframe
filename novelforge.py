...
CORPUS_TOOLS = {
    "scout": ROOT / "corpus" / "corpus_scout.py",
    "discovery": ROOT / "corpus" / "discovery_runtime.py",
    "rights": ROOT / "corpus" / "rights_gate.py",
}
...
def run_json(script: Path, args: list[str]) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(value, dict): raise ValueError("tool output must be object")
    except Exception:
...
def bootstrap(project_root: Path, task_mode: str, build: bool) -> dict[str, Any]:
    ...
    if code != 0 or not validation.get("valid"):
        return {"schema": "novelforge_bootstrap_v1", "ready": False, "task_mode": task_mode, "project_root": str(project_root), "validation": validation}
    build_result = None
    if build:
        code, build_result = run_json(PROJECT_ADAPTER, ["build", str(project_root)])
        if code != 0:
            return {"schema": "novelforge_bootstrap_v1", "ready": False, "task_mode": task_mode, "project_root": str(project_root), "validation": validation, "build_error": build_result}
    ...
    return {
        ...
        "build": build_result,
        "required_framework_reads": ["HARNESS_MANIFEST.yaml", "SKILL.md", "harness/HARNESS_AGENT.md"],
        "ai_native_policy": "Semantic story/character/reader/revision/research judgment belongs to the model through model-readable contracts; deterministic code owns only invariants, persistence, routing and transactions.",
        "semantic_entry": "novelforge.py semantic prepare-contract",
        ...
        "authority_policy": "Model outputs, reader diagnostics, memories, branches and semantic results never grant Canon write authority.",
    }


def doctor() -> dict[str, Any]:
    required = [PROJECT_SDK, PROJECT_ADAPTER, SESSION, MCP, LOCAL_AGENT, PEER_RELAY, *TOOLS.values(), *CORPUS_TOOLS.values()]
    missing = [str(path.relative_to(ROOT)) for path in dict.fromkeys(required) if not path.exists()]
    return {"schema": "novelforge_doctor_v1", "framework_version": FRAMEWORK_VERSION, "framework_root": str(ROOT), "ok": not missing, "missing": missing, "model_execution": False}
...
def self_test() -> int:
    checks: list[tuple[Path, list[str]]] = [
        ...
        (SESSION, ["self-test"]),
        (TOOLS["runtime"], ["--db", "/tmp/novelforge-cli-control.db", "self-test"]),
        (TOOLS["capabilities"], ["self-test"]),
        (TOOLS["context-inspect"], ["self-test"]),
        (TOOLS["memory-tiers"], ["self-test"]),
        (TOOLS["memory-bank"], ["--db", "/tmp/novelforge-cli-memory.db", "self-test", "--path", "/tmp/novelforge-cli-memory-selftest.db"]),
        (TOOLS["scenario-fork"], ["--db", "/tmp/novelforge-cli-scenario.db", "self-test", "--path", "/tmp/novelforge-cli-scenario-selftest.db"]),
        (SEMANTIC, ["self-test"]),
        (TOOLS["quality-findings"], ["self-test"]),
        (TOOLS["reader-expectation"], ["--db", "/tmp/novelforge-cli-reader-expectation.db", "self-test", "--path", "/tmp/novelforge-cli-reader-expectation-selftest.db"]),
        (TOOLS["quality-evolution"], ["--db", "/tmp/novelforge-cli-quality.db", "self-test", "--path", "/tmp/novelforge-cli-quality-selftest.db"]),
        (TOOLS["state-graph"], ["self-test"]),
        (TOOLS["learning"], ["--db", "/tmp/novelforge-cli-learning.db", "self-test"]),
        (TOOLS["learning-cycle"], ["self-test", "--path", "/tmp/novelforge-cli-learning-cycle.db"]),
        (TOOLS["learning-work"], ["self-test"]),
        (TOOLS["learning-gate"], ["self-test"]),
        (CORPUS_TOOLS["scout"], ["self-test"]),
        (CORPUS_TOOLS["discovery"], ["self-test"]),
        (CORPUS_TOOLS["rights"], ["self-test"]),
        (MCP, ["--self-test"]),
        (TOOLS["bundle"], ["self-test"]),
    ]
    results = []
    ok = True
    for script, argv in checks:
        proc = subprocess.run([sys.executable, str(script), *argv], text=True, capture_output=True, check=False)
        results.append({"script": str(script.relative_to(ROOT)), "returncode": proc.returncode, "stdout": proc.stdout.strip()[:4000], "stderr": proc.stderr.strip()[:2000]})
        ok = ok and proc.returncode == 0
    dump({"novelforge_cli_contract": "PASS" if ok else "FAIL", "framework_version": FRAMEWORK_VERSION, "checks": results, "ai_native_semantic_entry": "harness/semantic_workers/semantic_worker_router.py", "model_execution": False})
    return 0 if ok else 1


def main() -> int:
    parser = argparse.ArgumentParser(description="NovelForge AI-native Adaptive Fiction Agent Framework")
    sub = parser.add_subparsers(dest="cmd", required=True)

    boot = sub.add_parser("bootstrap")
    boot.add_argument("--project-root", required=True)
    boot.add_argument("--task-mode", required=True, choices=TASK_MODES)
    boot.add_argument("--no-build", action="store_true")
    ...
    if args.cmd == "bootstrap":
        value = bootstrap(Path(args.project_root), args.task_mode, not args.no_build); dump(value); return 0 if value["ready"] else 1
    if args.cmd == "project":
    ...
    if args.cmd in TOOLS:
        argv = args.args
        if not argv and args.cmd in {"semantic", "context-inspect", "memory-tiers", "quality-findings", "state-graph"}:
            argv = ["self-test"] if args.cmd != "semantic" else ["list-contracts"]
        return call(TOOLS[args.cmd], argv)
    if args.cmd == "doctor":
        value = doctor(); dump(value); return 0 if value["ok"] else 1
    return self_test()
