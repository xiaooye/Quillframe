#!/usr/bin/env python3
"""Deterministic Claude Code lifecycle hook → local Quillframe Control Plane.
...
def main()->int:
    try:
        ...
        cp=ControlPlane(os.getenv("QUILLFRAME_DB",str(ROOT/".quillframe"/"runtime.db")));cp.init()
        existing=cp.get_session(sid)
        payload=(dict(existing["session"]) if existing else {
            "session_id":sid,
            "resource_id":os.getenv("QUILLFRAME_RESOURCE_ID","RUNTIME-LOCAL"),
            "project_id":os.getenv("QUILLFRAME_PROJECT_ID"),
            "role":"manager",
            "status":"running",
        ...
        print(f"quillframe Claude hook warning: {type(exc).__name__}: {exc}",file=sys.stderr)
