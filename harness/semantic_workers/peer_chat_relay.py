...
def build(job:dict[str,Any])->dict[str,Any]:
    ...
    reviewer_instruction=(
        ...
        "Return ONLY one JSON semantic result with the exact job_id/subject_id/kind/input_fingerprint, status=completed, worker provider=chatgpt_peer_chat (or truthful peer-chat/automation provider), a short evidence-based judgment, empty proposals/errors, and execution.run_reference exactly equal to the relay nonce. "
...
def validate_peer_result(packet:dict[str,Any],result:dict[str,Any])->list[str]:
    ...
    if worker.get("provider") not in {"chatgpt_peer_chat","claude_peer_chat","gemini_peer_chat","github_copilot_actions","human","other_peer_chat"}:e.append("worker.provider is not a declared independent peer-chat/automation/human provider")
