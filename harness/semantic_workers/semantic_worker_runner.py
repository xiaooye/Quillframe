...
It dispatches validated jobs to an independent subprocess adapter. It does not
perform semantic judgment. Higher-level Control Plane/GitHub/chat transports
remain eligible when the direct layer has no adapter.
"""
...
def local_command()->tuple[str|None,str|None]:
    if os.getenv("NOVEL_OS_DISABLE_LOCAL_AGENT_AUTO","").lower() in {"1","true","yes"}:return None,None
    requested=os.getenv("NOVEL_OS_LOCAL_AGENT_PROVIDER","auto").strip().lower() or "auto"
    if requested not in {"auto","codex","claude"}:return None,None
    selected=None
    if requested=="codex" and shutil.which("codex"):selected="codex"
    elif requested=="claude" and shutil.which("claude"):selected="claude"
    elif requested=="auto":selected="codex" if shutil.which("codex") else ("claude" if shutil.which("claude") else None)
    if not selected:return None,None
    adapter=Path(__file__).resolve().parent/"adapters"/"local_agent_adapter.py"
    if not adapter.exists():return None,None
    return shlex.join([sys.executable,str(adapter),"--provider",selected]),f"local_{selected}_cli"

def openai_command()->str|None:
...
def resolve(explicit:str|None)->tuple[str|None,str|None]:
    ...
    if os.getenv("NOVEL_OS_SEMANTIC_WORKER_CMD"):return os.environ["NOVEL_OS_SEMANTIC_WORKER_CMD"],"environment"
    cmd,src=local_command()
    if cmd:return cmd,src
    cmd=openai_command()
    if cmd:return cmd,"openai_env"
    return None,None

def capabilities(cmd:str|None,src:str|None)->dict[str,Any]:
    return {"semantic_execution_runtime":"0.4","router_available":True,"command_adapter_supported":True,"adapter_configured":bool(cmd),"adapter_source":src,"local_codex_detected":shutil.which("codex") is not None,"local_claude_detected":shutil.which("claude") is not None,"built_in_model_provider":src=="openai_env","local_agent_provider":src in {"local_codex_cli","local_claude_cli"},"independent_worker_available":bool(cmd),"fallback_without_adapter":"semantic_pending_at_direct_layer","higher_level_transports":["control_plane_mcp","github_bridge","peer_chat_relay","human"]}
...
def invoke(job:dict[str,Any],cmd:str,timeout:int)->tuple[dict[str,Any]|None,dict[str,Any]]:
    ...
    except json.JSONDecodeError as exc:return None,{"state":"semantic_invalid","error":f"adapter stdout invalid JSON: {exc}"}
    if not isinstance(result,dict):return None,{"state":"semantic_invalid","error":"adapter result must be object"}
    errors=validate_result(job,result)
    if errors:return None,{"state":"semantic_invalid","error":"; ".join(errors)}
    if result.get("status")=="unsupported":return result,{"state":"unsupported"}
    if result.get("status")=="failed":return result,{"state":"worker_failed","error":"; ".join(result.get("errors",[])) or None}
    j=result.get("judgment",{}); rejected=j.get("verdict")=="reject" or j.get("result")=="fail"
...
def run_jobs(payload:dict[str,Any],cmd:str|None,src:str|None,timeout:int)->dict[str,Any]:
    ...
    overall="semantic_invalid" if "semantic_invalid" in states else ("worker_failed" if "worker_failed" in states else ("unsupported" if "unsupported" in states else ("semantic_reject" if "semantic_reject" in states else "completed")))
    return {"semantic_execution_version":"4","status":overall,"adapter":{"type":"command","source":src,"command":cmd},"direct_layer_only":True,"results":results,"executions":executions}
...
def main()->int:
    ...
    if args.cmd=="capabilities":dump(capabilities(cmd,src));return 0
    report=run_jobs(load_json(Path(args.jobs)),cmd,src,args.timeout);dump(report,Path(args.output) if args.output else None)
    if args.require_complete and report["status"] not in {"completed","semantic_reject"}:return 2
