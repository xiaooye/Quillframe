#!/usr/bin/env python3
"""Minimal stdio MCP adapter for Novel Production OS v6.6 Control Plane.
...
SERVER_INFO = {
    "name": "novel-os-control-plane",
    "title": "Novel Production OS Control Plane",
    "version": "6.6.0",
}

TOOLS: list[dict[str, Any]] = [
    {
        "name": "novel_os_status",
        "title": "Novel OS runtime status",
        "description": "Read operational Session/Event/Handoff store status. This is not Canon state.",
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
    },
    {
        "name": "novel_os_session_put",
        "title": "Persist Novel OS session",
        "description": "Persist an OS session snapshot with optional optimistic version check. Operational state only.",
        "inputSchema": {
            "type": "object",
            "properties": {"session": {"type": "object"}, "expected_version": {"type": ["integer", "null"], "minimum": 0}},
            "required": ["session"], "additionalProperties": False,
        },
    },
    {
        "name": "novel_os_session_get",
        "title": "Read Novel OS session",
        "description": "Read one persisted OS session by session_id.",
        "inputSchema": {"type": "object", "properties": {"session_id": {"type": "string", "minLength": 1}}, "required": ["session_id"], "additionalProperties": False},
    },
    {
        "name": "novel_os_session_list",
        "title": "List Novel OS sessions",
    ...
    {
        "name": "novel_os_event_ingest",
        "title": "Ingest Novel OS event",
        "description": "Idempotently ingest a typed novel_os_event_v1 request/observation/result. Events do not grant Canon authority.",
        "inputSchema": {"type": "object", "properties": {"event": {"type": "object"}}, "required": ["event"], "additionalProperties": False},
    },
    {
        "name": "novel_os_handoff_submit",
        "title": "Submit bounded handoff",
        "description": "Queue a typed bounded cross-session handoff. Canon/OS behavior/taste write permissions must remain false.",
        "inputSchema": {"type": "object", "properties": {"handoff": {"type": "object"}}, "required": ["handoff"], "additionalProperties": False},
    },
    {
        "name": "novel_os_handoff_claim",
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "target_session_class": {"type": ["string", "null"]},
    ...
    {
        "name": "novel_os_handoff_complete",
        ...
        "inputSchema": {
            ...
            "required": ["handoff_id", "worker_id", "result"], "additionalProperties": False,
        },
    },
    {
        "name": "novel_os_result_consume",
        ...
        "inputSchema": {
            ...
            "required": ["source_type", "source_id", "consumer", "payload_hash"], "additionalProperties": False,
        },
    },
]
...
class MCPServer:
    ...
    def call_tool(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        if name == "novel_os_status": return self.cp.status()
        if name == "novel_os_session_put": return self.cp.put_session(args["session"], expected_version=args.get("expected_version"))
        if name == "novel_os_session_get":
            value = self.cp.get_session(args["session_id"])
            if value is None: raise ValueError("session not found")
            return value
        if name == "novel_os_session_list": return {"sessions": self.cp.list_sessions(args.get("resource_id"))}
        if name == "novel_os_event_ingest": return self.cp.ingest_event(args["event"])
        if name == "novel_os_handoff_submit": return self.cp.submit_handoff(args["handoff"])
        if name == "novel_os_handoff_claim": return {"claim": self.cp.claim_handoff(args["worker_id"], target_session_class=args.get("target_session_class"), lease_seconds=int(args.get("lease_seconds", 300)))}
        if name == "novel_os_handoff_get":
            value = self.cp.get_handoff(args["handoff_id"])
            if value is None: raise ValueError("handoff not found")
            return value
        if name == "novel_os_handoff_complete": return self.cp.complete_handoff(args["handoff_id"], args["worker_id"], args["result"], failed=bool(args.get("failed", False)))
        if name == "novel_os_result_consume": return self.cp.consume_once(args["source_type"], args["source_id"], args["consumer"], args["payload_hash"])
        raise KeyError(name)
    ...
    def handle(self, msg: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if method == "initialize":
            self.initialized = False
            return {"jsonrpc": "2.0", "id": rid, "result": {"protocolVersion": PROTOCOL_VERSION, "capabilities": {"tools": {"listChanged": False}}, "serverInfo": SERVER_INFO, "instructions": "Operational Novel OS runtime tools only. They do not grant Canon or OS-behavior authority."}}
        if method == "ping": return {"jsonrpc": "2.0", "id": rid, "result": {}}
        if not self.initialized:
            return {"jsonrpc": "2.0", "id": rid, "error": {"code": -32002, "message": "server not initialized"}}
        ...
        if method == "tools/call":
            ...
            try:
                ...
                return {"jsonrpc": "2.0", "id": rid, "result": text_result(value)}
            except Exception as exc:
                return {"jsonrpc": "2.0", "id": rid, "result": text_result({"error": type(exc).__name__, "message": str(exc)}, is_error=True)}
...
def main() -> int:
    p = argparse.ArgumentParser(description="Novel OS MCP stdio control-plane adapter")
    p.add_argument("--db", default=os.getenv("NOVEL_OS_DB", ".novel-os/runtime.db"))
