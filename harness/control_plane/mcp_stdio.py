#!/usr/bin/env python3
"""Minimal stdio MCP adapter for the NovelForge Control Plane.
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "novelforge_status",
        "title": "NovelForge runtime status",
        "description": "Read operational Session/Event/Handoff store status. This is not Canon state.",
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
    },
    {
        "name": "novelforge_session_put",
        "title": "Persist NovelForge session",
        "description": "Persist a NovelForge session snapshot with optional optimistic version check. Operational state only.",
    ...
    {
        "name": "novelforge_session_get",
        "title": "Read NovelForge session",
        "description": "Read one persisted NovelForge session by session_id.",
        "inputSchema": {"type": "object", "properties": {"session_id": {"type": "string", "minLength": 1}}, "required": ["session_id"], "additionalProperties": False},
    },
    {
        "name": "novelforge_session_list",
        "title": "List NovelForge sessions",
    ...
    {
        "name": "novelforge_event_ingest",
        "title": "Ingest NovelForge event",
        "description": "Idempotently ingest a typed novelforge_event_v1 request/observation/result. Events do not grant Canon authority.",
        "inputSchema": {"type": "object", "properties": {"event": {"type": "object"}}, "required": ["event"], "additionalProperties": False},
    },
    {
        "name": "novelforge_handoff_submit",
    ...
    {
        "name": "novelforge_handoff_complete",
    ...
    {
        "name": "novelforge_result_consume",
...
class MCPServer:
    ...
    def call_tool(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        if name == "novelforge_status": return self.cp.status()
        if name == "novelforge_session_put": return self.cp.put_session(args["session"], expected_version=args.get("expected_version"))
        if name == "novelforge_session_get":
            value = self.cp.get_session(args["session_id"])
            if value is None: raise ValueError("session not found")
            return value
        if name == "novelforge_session_list": return {"sessions": self.cp.list_sessions(args.get("resource_id"))}
        if name == "novelforge_event_ingest": return self.cp.ingest_event(args["event"])
        if name == "novelforge_handoff_submit": return self.cp.submit_handoff(args["handoff"])
        if name == "novelforge_handoff_claim": return {"claim": self.cp.claim_handoff(args["worker_id"], target_session_class=args.get("target_session_class"), lease_seconds=int(args.get("lease_seconds", 300)))}
        if name == "novelforge_handoff_get":
            value = self.cp.get_handoff(args["handoff_id"])
            if value is None: raise ValueError("handoff not found")
            return value
        if name == "novelforge_handoff_complete": return self.cp.complete_handoff(args["handoff_id"], args["worker_id"], args["result"], failed=bool(args.get("failed", False)))
        if name == "novelforge_result_consume": return self.cp.consume_once(args["source_type"], args["source_id"], args["consumer"], args["payload_hash"])
        raise KeyError(name)
    ...
    def handle(self, msg: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if method == "initialize":
            self.initialized = False
            return {"jsonrpc": "2.0", "id": rid, "result": {"protocolVersion": PROTOCOL_VERSION, "capabilities": {"tools": {"listChanged": False}}, "serverInfo": SERVER_INFO, "instructions": "Operational NovelForge runtime tools only. They do not grant Canon or framework-behavior authority."}}
...
def main() -> int:
    p = argparse.ArgumentParser(description="NovelForge MCP stdio control-plane adapter")
