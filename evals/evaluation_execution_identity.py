...
def build_identity(*, root: Path, queue: Path, jobs: Path, capabilities: Path, candidate_commit: str,
    ...
    payload: dict[str, Any] = {
        ...
        "resource_budget": {
            ...
            "token_budget": None,
            "binding": "workflow_has_no_explicit_token_or_job-timeout_budget",
        },
        "provenance": {
...
def self_test() -> int:
    base = {
        ...
        "resource_budget": {"require_complete": True},
    ...
    ok = pristine and tamper_detected and rebound_valid and base["identity_fingerprint"] != rebound["identity_fingerprint"]
    print(json.dumps({"execution_identity_contract": "PASS" if ok else "FAIL", "tamper_detected": tamper_detected,
                      "model_change_changes_identity": base["identity_fingerprint"] != rebound["identity_fingerprint"]}, indent=2))
