...
check(!main.includes('import "./styles/start-hub.css"'), "retired start layout must not ship in the product bundle");
check(!main.includes('import "./styles/start-hub-kawaii.css"'), "retired start kawaii layer must not ship in the product bundle");
check(main.includes('if (path === "/start")') && main.includes('window.location.replace("/")'), "legacy /start deep links must collapse safely into product home");
check(!main.includes('new Set(["/start"'), "/start must not remain a standalone handoff target");
...
if (failures.length) {
  ...
  console.log(JSON.stringify({
    schema: "novelforge_start_flow_consolidation_quality_v1",
    status: "pass",
    standalone_start_page: false,
    legacy_start_redirect: "/",
    product_home_is_primary_entry: true,
    project_inspector_discoverable: true,
    local_playground_discoverable: true,
    agent_integration_discoverable: true,
  }, null, 2));
}
