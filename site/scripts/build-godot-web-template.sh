...
command -v emcc >/dev/null 2>&1 || { echo "emcc is required" >&2; exit 1; }
command -v scons >/dev/null 2>&1 || { echo "scons is required" >&2; exit 1; }
mkdir -p "${WORK_ROOT}" "${OUTPUT_DIR}"
rm -rf "${SOURCE_DIR}"

curl -fsSL "https://github.com/godotengine/godot/releases/download/${GODOT_RELEASE}/godot-${GODOT_RELEASE}.tar.xz" -o "${SOURCE_ARCHIVE}"
