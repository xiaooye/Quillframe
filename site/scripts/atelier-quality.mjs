...
check(!main.includes("appearance-migrated") && !main.includes("appearance-v5"), "appearance must not retain a state migration path");
check(main.includes('document.documentElement.dataset.experience = "story-loom-kawaii-atelier-v5"'), "v5 experience identity must be stamped on the document root");
...
check(!/requestAnimationFrame\s*\(|setInterval\s*\(/.test(`${app}\n${main}`), "unified Product Entry must not add decorative frame loops or polling");
...
if (failures.length) {
  ...
  console.log(JSON.stringify({ schema: "quillframe_product_atelier_quality_v5", status: "pass", identity: "story_loom_kawaii_atelier_v5", unified_shell: true, css_entrypoint: "index.css", css_cascade: "primitives_routes_shared_composition_readability", weiui_foundation: true, story_loom_lane_authority: true, route_wallpaper_layer_active: false, hosted_studio_preserved: true, canvas_first_composition: true, appearance_state_migration: false, progressive_css: true, infinite_idle_animation: false, authority: false }, null, 2));
}
