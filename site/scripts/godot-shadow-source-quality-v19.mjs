...
check(scene.includes('path="res://scripts/post_merge_stabilization.gd"'), "production scene must enter through post-merge stabilization");
...
check(!/Node3D|Camera3D|MeshInstance3D/.test(stabilization), "post-merge stabilization must remain within the 2.5D Product cap");
check(!stabilization.includes("Timer.new") && !stabilization.includes("setInterval") && !stabilization.includes("set_interval"), "post-merge stabilization must remain event-driven with no polling");
check(!stabilization.includes("SystemFont.new"), "Godot Web stabilization must not depend on host system fonts");

// Preserve every v18 source contract. v18's only stale assumption is that the
// scene directly enters responsive_completion.gd; v19 validates the new real
// entrypoint above, then runs the unchanged v18 suite against an isolated copy
// where Main.tscn is projected one inheritance level down. No repository file
// is edited and all other v18 source checks run unchanged.
...
try {
  ...
  const projectedScene = scene.replace('path="res://scripts/post_merge_stabilization.gd"', 'path="res://scripts/responsive_completion.gd"');
...
if (failures.length) {
  ...
  console.log(JSON.stringify({
    ...
    production_entrypoint: "post_merge_stabilization.gd",
