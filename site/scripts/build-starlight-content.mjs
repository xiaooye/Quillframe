...
function rewriteLinks(source, sourcePath, locale) {
  ...
  const rewrittenImages = source.replace(
    ...
    (full, alt, href) => {
      ...
      const resolved = path.posix.normalize(path.posix.join(sourceDir, decodeURIComponent(pathname)));
      const absolute = path.join(repoRoot, ...resolved.split("/"));

      if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile()) return full;
      ...
      return `![${alt}](${publicHref}${hash ? `#${hash}` : ""})`;
    },
  );

  return rewrittenImages.replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+["'][^"']*["'])?\)/g, (full, label, href) => {
    ...
    if (/\.md$/i.test(pathname)) {
      ...
      if (targetId) {
        return `[${label}](${pageHref(targetId, locale)}${hash ? `#${hash}` : ""})`;
      }
    }
