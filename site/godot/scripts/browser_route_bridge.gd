...
func _ready() -> void:
	...
	if _window == null or _document == null: return
...
func _exit_tree() -> void:
	if OS.has_feature("web") and _window != null and _popstate_callback != null:
...
func _on_popstate(_args: Array) -> void:
	...
	if is_instance_valid(host) and host.has_method("_navigate"):
		...
		for bridge_name in ["ThemeBridge", "LocaleBridge"]:
			var bridge = host.get_node_or_null(bridge_name)
			var method := "apply_story_loom_theme" if bridge_name == "ThemeBridge" else "apply_current_locale"
			if bridge != null and bridge.has_method(method): bridge.call_deferred(method)
	_publish_route(path)

func _browser_path() -> String:
	return "/" if _window == null else str(_window.location.pathname)

func _publish_route(path: String) -> void:
	if _document != null: _document.documentElement.dataset.novelforgeRoute = path
