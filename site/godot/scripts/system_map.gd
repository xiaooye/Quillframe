...
const NODES := [
	{"id": "project", "label": "PROJECT", "p": [0.10, 0.48], "depth": 0.35},
	{"id": "manager", "label": "MANAGER", "p": [0.28, 0.48], "depth": 0.55},
	{"id": "context", "label": "CONTEXT", "p": [0.47, 0.28], "depth": 0.85},
	{"id": "worker", "label": "WORKER", "p": [0.47, 0.68], "depth": 0.75},
	{"id": "agents", "label": "AGENTS", "p": [0.67, 0.18], "depth": 1.05},
	{"id": "gate", "label": "GATE", "p": [0.67, 0.48], "depth": 1.10},
	{"id": "inspector", "label": "INSPECT", "p": [0.67, 0.78], "depth": 0.95},
	{"id": "settlement", "label": "SETTLE", "p": [0.84, 0.48], "depth": 1.25},
	{"id": "publication", "label": "PUBLISH", "p": [0.93, 0.68], "depth": 1.35},
]
...
func _process(delta: float) -> void:
	...
	if size.x > 0.0 and size.y > 0.0:
		...
		var target := (mouse - center) / maxf(size.x, size.y) * 22.0
...
func _draw() -> void:
	...
	_draw_grid(56.0, GRID_FINE, _parallax * 0.18)
	_draw_grid(168.0, GRID, _parallax * 0.34)
	...
	draw_string(font, Vector2(24, size.y - 24), "LIVE SYSTEM MAP  ·  drag-free spatial runtime", HORIZONTAL_ALIGNMENT_LEFT, -1, 12, MUTED)
...
func _draw_packet(a: Vector2, b: Vector2, from_id: String, to_id: String, hot: bool) -> void:
	...
	draw_circle(p, 5.5 if hot else 3.0, Color(glow.r, glow.g, glow.b, 0.14))
...
func _draw_node(node: Dictionary) -> void:
	...
	var lift := Vector2(5.0, 7.0) * depth
	...
	var port := Vector2(rect.position.x + 14.0, rect.get_center().y)
	draw_circle(port, 4.0, ACCENT if is_hot else MUTED)
	if is_hot:
		draw_circle(port, 9.0, Color(ACCENT.r, ACCENT.g, ACCENT.b, 0.08))
...
func _node_rect(node: Dictionary) -> Rect2:
	...
	var w := clampf(size.x * 0.13, 116.0, 172.0)
	var h := 58.0
...
func _node_at(position: Vector2) -> String:
	for index in range(NODES.size() - 1, -1, -1):
		...
		if _node_rect(node).grow(4.0).has_point(position):
			return str(node["id"])
	return ""
