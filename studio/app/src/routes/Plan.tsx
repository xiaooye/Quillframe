  ...
  return (
    <section class="nf-page qf-authoring-secondary">
      ...
      <Show when={project()} fallback={<div class="qf-empty-workspace"><strong>{zh() ? "打开 Project 后查看" : "Open a Project to continue"}</strong></div>}>
        {(value) => <div class="qf-editorial-sheet"><span class="nf-eyebrow">CORE SNAPSHOT</span><h2>{value().manifest.title}</h2><div class="qf-authoring-counts"><span><strong>{value().counts.plans ?? 0}</strong>{zh() ? "计划" : "plans"}</span><span><strong>{value().counts.story_nodes ?? 0}</strong>{zh() ? "故事节点" : "story nodes"}</span></div><aside class="qf-awaiting-core" role="status"><div><strong>awaiting_external</strong><code>plan.inspect / plan mutation contracts</code></div><p>{zh() ? "需要 Core 返回 typed Plan + Scene Card projection，并为每一种写操作定义 authority/CAS/idempotency。" : "Core must expose typed Plan + Scene Card projections and operation-specific authority/CAS/idempotency for writes."}</p></aside></div>}
      </Show>
    </section>
  );
}
