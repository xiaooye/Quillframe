...
check(experience.includes("Studio parity pass") && experience.includes("same soft NovelForge atmosphere"), "Studio parity layer must document its workstation-only ownership");
check(experience.includes("var(--nf-product-pink-soft)") && experience.includes("var(--nf-product-lilac-soft)") && experience.includes("var(--nf-product-mint-soft)"), "Studio parity layer must consume shared NovelForge color language rather than inventing local colors");
...
if(failures.length){
  ...
  console.log(JSON.stringify({schema:"novelforge_studio_product_language_v5",status:"pass",shared_tokens:true,single_css_entrypoint:true,legacy_visual_fixes_in_cascade:false,workstation_parity:true,hardening_layer:"final",product_language:"novelforge-kawaii-v1"},null,2));
}
