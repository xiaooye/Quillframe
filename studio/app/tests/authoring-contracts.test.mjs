...
test("missing Core primitives have explicit consumer requirements", () => {
  const names = new Set(contracts.CORE_CONSUMER_REQUIREMENTS.map((item) => item.operation));
  for (const required of ["document.get", "model.connect", "author.run.execute", "run.events.list", "candidate.review.get", "settlement.preflight"]) {
